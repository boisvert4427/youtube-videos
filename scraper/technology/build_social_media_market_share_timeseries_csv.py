...
DEFAULT_RAW_DIR = PROJECT_ROOT / "data" / "raw" / "technology" / "social_media_market_share"
DEFAULT_RAW_CSV = DEFAULT_RAW_DIR / "statcounter_social_media_market_share_monthly.csv"
...
DEFAULT_OUTPUT = (
    ...
    / "social_media_market_share_2009_2026.csv"
)
...
START_MONTH = "2009-04"
END_MONTH = "2026-05"
USER_AGENT = "Mozilla/5.0 (compatible; Codex Social Media Market Share Builder)"
...
def _platform_key(name: str) -> str:
    aliases = {
        "Google+": "google",
        "NowPublic": "newspublic",
        "iWiW": "iwiw",
        "news.ycombinator.com": "news_ycombinator_com",
        "reddit": "reddit",
    }
    if name in aliases:
        return aliases[name]
...
def build_rows(raw_csv: Path) -> list[dict[str, str]]:
    with raw_csv.open("r", newline="", encoding="utf-8-sig") as file:
        ...
        platform_names = [name for name in (reader.fieldnames or []) if name != "Date"]
    if not wide_rows or not platform_names:
        raise RuntimeError("The StatCounter social media CSV is empty.")
    ...
    for wide_row in wide_rows:
        ...
        shares = {
            platform: max(0.0, float(wide_row.get(platform, "0") or 0))
        ...
        gains = sorted(
            (
                (share - previous_shares.get(platform, share), platform)
        ...
        if previous_shares and gains and gains[0][0] > 0:
            ...
            summary = f"Leader: {leader_name} {leader_share:.1f}%|Worldwide social media"

        for platform, share in ranked:
            previous = previous_shares.get(platform)
            output.append(
                {
                    "ranking_date": f"{month}-01",
                    "browser_name": platform,
                    "browser_key": _platform_key(platform),
...
def download_logos(logos_dir: Path, refresh: bool) -> list[Path]:
    ...
    for key, domain in LOGO_DOMAINS.items():
        ...
        if output_path.exists() and not refresh:
            continue
        try:
            _download(f"https://www.google.com/s2/favicons?domain={domain}&sz=128", output_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build worldwide social media market share data.")
...
def main() -> None:
    ...
    print(f"[scraper] Social media market share CSV generated -> {output}")
    print(
