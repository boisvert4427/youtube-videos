...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "shaq_vs_hakeem_career_shorts.mp4"
...
SHAQ_BLUE = (70, 146, 255)
HAKEEM_RED = (214, 58, 72)
GOLD = (255, 214, 134)
WHITE = (244, 242, 236)
TRACK = (78, 84, 94, 92)
...
STATS = [
    StatRow("TITLES", 4, 2, False),
    StatRow("MVP", 1, 1, False),
    StatRow("POINTS", 28596, 26946, False),
    StatRow("PTS/G", 23.7, 21.8, False),
    StatRow("REBOUNDS", 13099, 13748, False),
    StatRow("REB/G", 10.9, 11.1, False),
    StatRow("ASSISTS", 3026, 3058, False),
    StatRow("AST/G", 2.5, 2.5, False),
    StatRow("STEALS", 739, 2162, False),
    StatRow("STL/G", 0.6, 1.7, False),
    StatRow("BLOCKS", 2732, 3830, False),
    StatRow("BLK/G", 2.3, 3.1, False),
    StatRow("ALL-STAR", 15, 12, False),
    StatRow("1ST TEAM", 8, 6, False),
    StatRow("DPOY", 0, 2, False),
]
...
def _make_background() -> Image.Image:
    ...
    blue = np.array(SHAQ_BLUE, dtype=np.float32)
    red = np.array(HAKEEM_RED, dtype=np.float32)
...
def _make_frame_factory(assets_dir: Path, duration: float):
    ...
    shaq_source = assets_dir / "shaq_wiki.jpg"
    if not shaq_source.exists():
        shaq_source = assets_dir / "lebron_action.jpg"
    shaq = _load_player_cutout(shaq_source, SHAQ_BLUE)
    hakeem_source = assets_dir / "hakeem_wiki.jpg"
    if not hakeem_source.exists():
        hakeem_source = assets_dir / "jordan_action.jpg"
    hakeem = _load_player_cutout(hakeem_source, HAKEEM_RED)
    ...
    _draw_glow_text(title_layer, (292, 92), "SHAQ", title_font, WHITE, SHAQ_BLUE)
    _draw_glow_text(title_layer, (540, 98), "vs", vs_font, GOLD, GOLD, stroke_width=1)
    _draw_glow_text(title_layer, (790, 92), "HAKEEM", title_font, WHITE, HAKEEM_RED)
    ...
    def cached_energy(step: int) -> Image.Image:
        ...
        ed.ellipse((-80, 200, 390, 860), fill=(*SHAQ_BLUE, int(26 + 36 * pulse_left)))
        ed.ellipse((690, 200, 1160, 860), fill=(*HAKEEM_RED, int(26 + 36 * pulse_right)))
        for idx in range(20):
    ...
    def make_frame(t: float) -> np.ndarray:
        ...
        for idx, stat in enumerate(STATS):
            ...
            d.rounded_rectangle((left_x0, y + 22, left_x1, y + 40), radius=9, fill=(*SHAQ_BLUE, 245 if winner == "left" else 205))
            d.rounded_rectangle((right_x0, y + 22, right_x1, y + 40), radius=9, fill=(*HAKEEM_RED, 245 if winner == "right" else 205))
            ...
            if winner in {"left", "right"} and local_t > 0.6:
                ...
                marker_color = SHAQ_BLUE if winner == "left" else HAKEEM_RED
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Shaq vs Hakeem career Shorts with the cinematic board template.")
    ...
    try:
        ...
        rel = output
    print(f"[video_generator] Shaq vs Hakeem career Shorts generated -> {rel}")
