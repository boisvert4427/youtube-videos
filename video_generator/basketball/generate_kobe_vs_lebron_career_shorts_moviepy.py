...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "kobe_vs_lebron_career_shorts.mp4"
...
KOBE_GOLD = (241, 196, 72)
LEBRON_RED = (214, 58, 72)
GOLD = (255, 214, 134)
WHITE = (244, 242, 236)
TRACK = (78, 84, 94, 92)
...
STATS = [
    StatRow("TITLES", 5, 4, False),
    StatRow("MVP", 1, 4, False),
    StatRow("POINTS", 33643, 43180, False),
    StatRow("PTS/G", 25.0, 26.8, False),
    StatRow("REBOUNDS", 7047, 11997, False),
    StatRow("REB/G", 5.2, 7.5, False),
    StatRow("ASSISTS", 6306, 11909, False),
    StatRow("AST/G", 4.7, 7.4, False),
    StatRow("STEALS", 1944, 2399, False),
    StatRow("STL/G", 1.4, 1.5, False),
    StatRow("BLOCKS", 640, 1179, False),
    StatRow("BLK/G", 0.5, 0.7, False),
    StatRow("ALL-STAR", 18, 21, False),
    StatRow("1ST TEAM", 11, 13, False),
    StatRow("FINALS MVP", 2, 4, False),
]
...
DECIMAL_LABELS = {"PTS/G", "REB/G", "AST/G", "STL/G", "BLK/G"}
...
def _make_background() -> Image.Image:
    ...
    blue = np.array(KOBE_GOLD, dtype=np.float32)
    red = np.array(LEBRON_RED, dtype=np.float32)
...
def _make_frame_factory(assets_dir: Path, duration: float):
    ...
    kobe_source = assets_dir / "kobe_wiki.jpg"
    if not kobe_source.exists():
        kobe_source = assets_dir / "jordan_action.jpg"
    kobe = _load_player_cutout(kobe_source, KOBE_GOLD)
    lebron_source = assets_dir / "lebron_action.jpg"
    if not lebron_source.exists():
        lebron_source = assets_dir / "lebron_portrait.jpg"
    lebron = _load_player_cutout(lebron_source, LEBRON_RED)
    ...
    _draw_glow_text(title_layer, (292, 92), "KOBE", title_font, WHITE, KOBE_GOLD)
    _draw_glow_text(title_layer, (540, 98), "vs", vs_font, GOLD, GOLD, stroke_width=1)
    _draw_glow_text(title_layer, (790, 92), "LEBRON", title_font, WHITE, LEBRON_RED)
    ...
    def cached_energy(step: int) -> Image.Image:
        ...
        ed.ellipse((-80, 200, 390, 860), fill=(*KOBE_GOLD, int(26 + 36 * pulse_left)))
        ed.ellipse((690, 200, 1160, 860), fill=(*LEBRON_RED, int(26 + 36 * pulse_right)))
        for idx in range(20):
    ...
    def make_frame(t: float) -> np.ndarray:
        ...
        for idx, stat in enumerate(STATS):
            ...
            d.rounded_rectangle((left_x0, y + 22, left_x1, y + 40), radius=9, fill=(*KOBE_GOLD, 245 if winner == "left" else 205))
            d.rounded_rectangle((right_x0, y + 22, right_x1, y + 40), radius=9, fill=(*LEBRON_RED, 245 if winner == "right" else 205))
            ...
            if winner in {"left", "right"} and local_t > 0.6:
                ...
                marker_color = KOBE_GOLD if winner == "left" else LEBRON_RED
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Kobe vs LeBron career Shorts with the cinematic board template.")
    ...
    try:
        ...
        rel = output
    print(f"[video_generator] Kobe vs LeBron career Shorts generated -> {rel}")
