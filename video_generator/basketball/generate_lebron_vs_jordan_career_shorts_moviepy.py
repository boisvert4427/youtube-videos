...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "lebron_vs_jordan_career_shorts.mp4"
...
STATS = [
    StatRow("TITLES", 4, 6, False),
    StatRow("MVP", 4, 5, False),
    StatRow("POINTS", 43180, 32292, False),
    StatRow("PTS/G", 26.8, 30.1, False),
    StatRow("REBOUNDS", 11997, 6672, False),
    StatRow("REB/G", 7.5, 6.2, False),
    StatRow("ASSISTS", 11909, 5633, False),
    StatRow("AST/G", 7.4, 5.3, False),
    StatRow("STEALS", 2399, 2514, False),
    StatRow("STL/G", 1.5, 2.3, False),
    StatRow("BLOCKS", 1179, 893, False),
    StatRow("BLK/G", 0.7, 0.8, False),
    StatRow("ALL-STAR", 21, 14, False),
    StatRow("1ST TEAM", 13, 10, False),
    StatRow("DPOY", 0, 1, False),
]
...
def _make_frame_factory(assets_dir: Path, duration: float):
    ...
    lebron = _load_player_cutout(assets_dir / "lebron_action.jpg", LEBRON_BLUE)
    jordan_source = assets_dir / "jordan_basketballnetwork.jpg"
    if not jordan_source.exists():
        jordan_source = assets_dir / "jordan_radiofrance.jpg"
    if not jordan_source.exists():
        jordan_source = assets_dir / "jordan_1991_color.jpg"
    if not jordan_source.exists():
        jordan_source = assets_dir / "jordan_action.jpg"
    jordan = _load_player_cutout(jordan_source, JORDAN_RED)
    ...
    _draw_glow_text(title_layer, (292, 92), "LEBRON", title_font, WHITE, LEBRON_BLUE)
    _draw_glow_text(title_layer, (540, 98), "vs", vs_font, GOLD, GOLD, stroke_width=1)
    _draw_glow_text(title_layer, (790, 92), "JORDAN", title_font, WHITE, JORDAN_RED)
    ...
    def cached_energy(step: int) -> Image.Image:
        ...
        ed.ellipse((-80, 200, 390, 860), fill=(*LEBRON_BLUE, int(26 + 36 * pulse_left)))
        ed.ellipse((690, 200, 1160, 860), fill=(*JORDAN_RED, int(26 + 36 * pulse_right)))
        for idx in range(20):
    ...
    def make_frame(t: float) -> np.ndarray:
        ...
        for idx, stat in enumerate(STATS):
            ...
            d.rounded_rectangle((left_x0, y + 22, left_x1, y + 40), radius=9, fill=(*LEBRON_BLUE, 245 if winner == "left" else 205))
            d.rounded_rectangle((right_x0, y + 22, right_x1, y + 40), radius=9, fill=(*JORDAN_RED, 245 if winner == "right" else 205))
            ...
            if winner in {"left", "right"} and local_t > 0.6:
                ...
                marker_color = LEBRON_BLUE if winner == "left" else JORDAN_RED
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate LeBron vs Jordan career Shorts with the cinematic board template.")
    ...
    try:
        ...
        rel = output
    print(f"[video_generator] LeBron vs Jordan career Shorts generated -> {rel}")
