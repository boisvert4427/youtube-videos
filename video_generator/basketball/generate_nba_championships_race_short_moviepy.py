...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "nba_championships_race_short_1947_2025_80s.mp4"
...
def _make_team_badge(size: int, team_name: str, team_abbr: str) -> Image.Image:
    ...
    outer = _mix_rgb(primary, (0, 0, 0), 0.28)
    layer.rounded_rectangle((2, 2, size + 6, size + 6), radius=18, fill=(10, 20, 42, 236), outline=outer + (255,), width=2)
    layer.rounded_rectangle((6, 6, size + 2, size + 2), radius=14, fill=(255, 255, 255, 18))

    logo_path = PROJECT_ROOT / "data" / "raw" / "nba_team_logos" / f"{team_name.lower().replace(' ', '_').replace('.', '').replace('-', '_')}.png"
    if logo_path.exists():
        logo = Image.open(logo_path).convert("RGBA")
        logo = ImageOps.contain(logo, (size - 12, size - 12))
        x = (badge.width - logo.width) // 2
        y = (badge.height - logo.height) // 2
