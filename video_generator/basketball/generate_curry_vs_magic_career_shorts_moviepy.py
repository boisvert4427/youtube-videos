...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "curry_vs_magic_career_shorts.mp4"
...
STATS = [
    StatRow("TITLES", 4, 5, False),
    StatRow("MVP", 2, 3, False),
    StatRow("POINTS", 26476, 17707, False),
    StatRow("PTS/G", 24.8, 19.5, False),
    StatRow("REBOUNDS", 4959, 6559, False),
    StatRow("REB/G", 4.7, 7.2, False),
    StatRow("ASSISTS", 6733, 10141, False),
    StatRow("AST/G", 6.3, 11.2, False),
    StatRow("STEALS", 1598, 1724, False),
    StatRow("STL/G", 1.5, 1.9, False),
    StatRow("3PM", 4238, 325, False),
    StatRow("3P%", 42.2, 30.3, False),
    StatRow("ALL-STAR", 12, 12, False),
    StatRow("1ST TEAM", 4, 9, False),
    StatRow("FINALS MVP", 1, 3, False),
]
...
def _make_frame_factory(assets_dir: Path, duration: float):
    ...
    curry_source = assets_dir / "curry_wiki.jpg"
    ...
    _draw_glow_text(title_layer, (292, 92), "CURRY", title_font, WHITE, CURRY_BLUE)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Curry vs Magic career Shorts with the cinematic board template.")
    ...
    try:
        ...
        rel = output
    print(f"[video_generator] Curry vs Magic career Shorts generated -> {rel}")
