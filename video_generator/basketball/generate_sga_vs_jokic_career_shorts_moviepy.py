...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "sga_vs_jokic_career_shorts.mp4"
DEFAULT_PREVIEW_OUTPUT = PROJECT_ROOT / "data" / "processed" / "basketball" / "sga_vs_jokic_career_preview.mp4"
...
STATS = [
    StatRow("POINTS / GAME", 31.1, 27.7, False),
    StatRow("ASSISTS / GAME", 6.6, 10.7, False),
    StatRow("REBOUNDS / GAME", 4.3, 12.9, False),
    StatRow("FG% / TS%", 55.3, 56.9, False),
    StatRow("FT%", 89.8, 82.1, False),
    StatRow("BLOCKS / GAME", 0.8, 0.9, False),
    StatRow("STEALS / GAME", 1.7, 1.8, False),
    StatRow("AST/TOV", 2.3, 3.7, False),
    StatRow("PER", 30.5, 32.7, False),
    StatRow("GAMES PLAYED", 68, 65, False),
    StatRow("WIN %", 78.0, 65.9, False),
    StatRow("30+ POINT GAMES", 40, 28, False),
    StatRow("TRIPLE-DOUBLES", 0, 25, False),
]
...
def _make_background() -> Image.Image:
    ...
    red = np.array(JOKIC_GOLD, dtype=np.float32)
...
def _make_frame_factory(assets_dir: Path, duration: float):
    ...
    sga_source: Path | None = None
    for candidate in ("sga.png", "shai.png", "shai_gilgeous_alexander.png", "shai_gilgeous_alexander.jpg", "shai_gilgeous_alexander.webp"):
        path = assets_dir / candidate
        if path.exists():
            sga_source = path
            break
    jokic_source: Path | None = None
    for candidate in ("jokic.png", "nikola_jokic.png", "nikola_jokic.jpg", "nikola_jokic.webp"):
        path = assets_dir / candidate
        if path.exists():
            jokic_source = path
            break
    sga = _load_player_cutout(sga_source, SGA_BLUE) if sga_source is not None else _make_placeholder_cutout("SGA", SGA_BLUE)
    jokic = _load_player_cutout(jokic_source, JOKIC_GOLD) if jokic_source is not None else _make_placeholder_cutout("NJ", JOKIC_GOLD)
    ...
    _draw_glow_text(title_layer, (292, 92), "SGA", title_font, WHITE, SGA_BLUE)
    _draw_glow_text(title_layer, (540, 98), "vs", vs_font, GOLD, GOLD, stroke_width=1)
    _draw_glow_text(title_layer, (790, 92), "JOKIC", title_font, WHITE, JOKIC_GOLD)
    td.line((160, 142, 330, 142), fill=(255, 214, 134, 110), width=2)
    td.line((750, 142, 920, 142), fill=(255, 214, 134, 110), width=2)
    td.text((540, 137), "MVP BATTLE", font=sub_font, fill=(*WHITE, 240), anchor="mm")
    ...
    def cached_energy(step: int) -> Image.Image:
        ...
        ed.ellipse((-80, 200, 390, 860), fill=(*SGA_BLUE, int(26 + 36 * pulse_left)))
        ed.ellipse((690, 200, 1160, 860), fill=(*JOKIC_GOLD, int(26 + 36 * pulse_right)))
        for idx in range(20):
    ...
    def make_frame(t: float) -> np.ndarray:
        ...
        for idx, stat in enumerate(STATS):
            ...
            d.rounded_rectangle((left_x0, y + 22, left_x1, y + 40), radius=9, fill=(*SGA_BLUE, 245 if winner == "left" else 205))
            d.rounded_rectangle((right_x0, y + 22, right_x1, y + 40), radius=9, fill=(*JOKIC_GOLD, 245 if winner == "right" else 205))
            ...
            if winner in {"left", "right"} and local_t > 0.6:
                ...
                marker_color = SGA_BLUE if winner == "left" else JOKIC_GOLD
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate SGA vs Jokic career Shorts with the cinematic board template.")
    ...
    try:
        ...
        rel = output
    print(f"[video_generator] SGA vs Jokic career Shorts generated -> {rel}")
