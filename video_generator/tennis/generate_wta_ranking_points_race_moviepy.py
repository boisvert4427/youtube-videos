...
DEFAULT_INPUT = PROJECT_ROOT / "data" / "processed" / "tennis" / "wta_rankings_weekly_top12_api_only_2000_2026.csv"
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "tennis" / "wta_ranking_points_race_2000_2026_38min_60fps_1080p.mp4"
DEFAULT_PREVIEW = PROJECT_ROOT / "data" / "processed" / "tennis" / "wta_ranking_points_race_preview.png"
DEFAULT_BACKGROUND_IMAGE = Path(r"C:\Users\leona\Downloads\wta_grand_slam_background_3840x2160.png")
DEFAULT_WIMBLEDON_LOGO = None
...
TITLE = "WTA RANKINGS RACE"
SUBTITLE = "WOMEN'S SINGLES - OFFICIAL WTA RANKING POINTS | 2000-2026"
NORMALIZED_SEASON_SUMMARY = "Official WTA API only|Weekly top 12|2000-2026"
...
WIMBLEDON_COLORS = {
    ...
    "Mirra Andreeva": "#e9dfc6",
    "Aryna Sabalenka": "#663c96",
    "Coco Gauff": "#0d7047",
    "Jessica Pegula": "#eadbb5",
    "Amanda Anisimova": "#6a3b98",
    "Elena Rybakina": "#d4af5b",
    "Caroline Wozniacki": "#c7b88a",
    "Maria Sharapova": "#eadbb5",
    "Simona Halep": "#0b6642",
    "Martina Hingis": "#efe2c7",
    "Lindsay Davenport": "#7044a8",
...
def _make_background(background_image: Path | None = None) -> Image.Image:
    if background_image is not None and background_image.exists():
        try:
            ...
            vdraw.rectangle((0, 0, WIDTH, HEIGHT), fill=(0, 0, 0, 26))
            vdraw.rectangle((0, 0, WIDTH, 190), fill=(0, 0, 0, 46))
            vdraw.rectangle((0, 190, 1180, HEIGHT), fill=(0, 0, 0, 20))
            vdraw.rectangle((1180, 190, WIDTH, HEIGHT), fill=(0, 0, 0, 36))
            vdraw.ellipse((-260, 690, 760, 1250), fill=(0, 0, 0, 30))
...
def _normalize_input_csv(input_csv: Path) -> Path:
    with input_csv.open("r", newline="", encoding="utf-8") as file:
        reader = csv.DictReader(file)
        fieldnames = [name.strip() for name in (reader.fieldnames or [])]
        if "points" in fieldnames:
            return input_csv
        if "ranking_points" not in fieldnames:
            raise RuntimeError(f"Unsupported WTA input format: {input_csv}")
        rows = list(reader)

    normalized_path = input_csv.with_name(f"{input_csv.stem}_normalized_points.csv")
    normalized_path.parent.mkdir(parents=True, exist_ok=True)
    with normalized_path.open("w", newline="", encoding="utf-8") as file:
        ...
        for row in rows:
            ...
            ranking_points = row.get("ranking_points", "").strip()
            if not ranking_date or not player_name or not ranking_points:
                continue
            writer.writerow(
                {
                    ...
                    "season_summary": NORMALIZED_SEASON_SUMMARY,
                }
            )
...
def render_video(
    ...
    input_csv = _normalize_input_csv(input_csv)
    ...
    intro_snapshot = first_snapshot.__class__(
        ...
        season_summary="Official WTA ranking points|Year-end snapshots + latest ranking|WTA Media rankings",
        states=[],
    )
    ...
    def render_frame(t: float) -> Image.Image:
        ...
        title_x = logo_x + (98 if logo_image is not None else 0)
        ...
        month_year_text = datetime.fromisoformat(nxt.ranking_date).strftime("%b %Y").upper()
        year_pad = 30
        year_font_fit = _fit_font_size(draw, month_year_text, 360, 64, 40, bold=True)
        year_bbox = draw.textbbox((0, 0), month_year_text, font=year_font_fit)
        ...
        for state in top_states:
            ...
            # Linear movement keeps rows exiting the top 12 without the tiny
            # pause that can happen with delayed easing near the bottom.
            y_idx = float(prev_idx) + (float(next_idx) - float(prev_idx)) * alpha
        ...
        for _, y, state, bar_w in items:
            ...
            value_text = f"{int(round(state.titles)):,}".replace(",", " ")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a WTA ranking points race video.")
...
def main() -> None:
    ...
    if args.preview is not None:
        print(f"[video_generator] WTA ranking preview generated -> {output}")
    else:
        print(f"[video_generator] WTA ranking race generated -> {output}")
