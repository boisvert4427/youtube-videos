...
DEFAULT_INPUT = PROJECT_ROOT / "data" / "processed" / "tennis" / "monte_carlo_titles_timeseries_1968_2025.csv"
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "processed" / "tennis" / "monte_carlo_titles_shorts_1968_2025_50s.mp4"

TITLE = "MONTE-CARLO TITLES"
SUBTITLE = "ATP men · cumulative champions race"
FPS = DEFAULT_FPS
TOTAL_DURATION = 50.0


def _monte_carlo_background() -> Image.Image:
    frame = _make_background()
    overlay = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay, "RGBA")
    draw.ellipse((128, 112, WIDTH - 128, 752), outline=(129, 212, 255, 18), width=4)
    draw.line((104, 1464, WIDTH - 104, 1464), fill=(234, 111, 74, 26), width=4)
    draw.line((168, 1544, WIDTH - 168, 1544), fill=(255, 255, 255, 12), width=2)
    draw.arc((96, 900, WIDTH - 96, 1768), start=196, end=344, fill=(255, 182, 120, 18), width=3)
    warm = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 0))
    warm_draw = ImageDraw.Draw(warm, "RGBA")
    warm_draw.ellipse((210, 1180, WIDTH - 210, HEIGHT + 140), fill=(177, 76, 44, 30))
    warm = warm.filter(ImageFilter.GaussianBlur(radius=58))
    overlay = overlay.filter(ImageFilter.GaussianBlur(radius=3))
    frame.alpha_composite(warm)
    frame.alpha_composite(overlay)
    return frame


def _monte_carlo_color(name: str) -> str:
    special = {
        "Rafael Nadal": "#d85c3c",
        "Stefanos Tsitsipas": "#5db7ff",
        "Novak Djokovic": "#27b36a",
        "Bjorn Borg": "#f1cd55",
        "Ilie Nastase": "#8c7cff",
        "Thomas Muster": "#ff8f5e",
        "Guillermo Vilas": "#39c08f",
        "Mats Wilander": "#51c3e6",
        "Ivan Lendl": "#df6cf0",
        "Carlos Alcaraz": "#ffb347",
        "Andrey Rublev": "#f25f5c",
        "Stan Wawrinka": "#efeff2",
        "Fabio Fognini": "#89d67c",
        "Juan Carlos Ferrero": "#e3b24a",
        "Gustavo Kuerten": "#61d1ff",
    }
    return special.get(name, PLAYER_COLORS.get(name, "#53c7ff"))
...
def render_video(
    ...
    if len(snapshots) < 2:
        raise RuntimeError("Not enough Monte-Carlo snapshots to render.")
    ...
    background = _monte_carlo_background()
    ...
    def make_frame(t: float) -> np.ndarray:
        ...
        draw.rounded_rectangle(header, radius=42, fill=(9, 26, 47, 216), outline=(255, 255, 255, 18), width=1)
        draw.text((76, 74), TITLE, font=title_font, fill="#f8f2ea")
        draw.text((78, 146), SUBTITLE, font=subtitle_font, fill="#cfe4f3")
        draw.rounded_rectangle((72, 196, 260, 294), radius=30, fill=(222, 102, 64, 255))
        year_bbox = draw.textbbox((0, 0), str(nxt.year), font=year_font)
        draw.text((166 - (year_bbox[2] - year_bbox[0]) // 2, 204), str(nxt.year), font=year_font, fill="#fff4ec")
        ...
        draw.rounded_rectangle(summary_rect, radius=28, fill=(14, 54, 84, 226), outline=(255, 179, 120, 80), width=2)
        ...
        for lane_index in range(top_n):
            ...
            draw.rounded_rectangle((rank_x, rank_top, rank_x + 54, rank_bottom), radius=18, fill=(222, 102, 64, 255))
        ...
        for _, y, state, bar_w in items:
            y0 = int(y)
            color = _monte_carlo_color(state.player_name)
            ...
            _draw_glow_text(frame, (value_x, bar_top + (bar_height - 30) // 2), value_text, value_font, (248, 242, 234), (255, 179, 120))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Monte-Carlo Shorts bar chart race.")
...
def main() -> None:
    ...
    print(f"[video_generator] Monte-Carlo Shorts race generated -> {output}")
